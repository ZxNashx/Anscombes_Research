...
x = np.arange(4, 16)  # x values from 4 to 15
M = np.column_stack((np.sin(x), np.ones(len(x))))
...
equation1 = sp.Eq(result.dot(ones_vector), 11 * ybar)
solution1 = sp.solve(equation1, (a, b))
print("\n\nTHE RESULTING VECTOR 2.1")
print(solution1)

#stage 2.2
modified_vector = result - 7.5 * ones_vector

modified_dot_product = modified_vector.dot(modified_vector)

equation2 = sp.Eq(modified_vector.dot(modified_vector), 10 * (33/8))

solution2 = sp.solve(equation2, (a, b))
...
equation3 = sp.Eq(modified_x_vector.dot(modified_result_vector), 10 * 11 * 1/2)
solution3 = sp.solve(equation3, (a, b))
print("\n\nTHE RESULTING VECTOR 2.3")
print(solution3)

solution_final = sp.solve((equation1, equation2, equation3), (a, b))
...
for s in solution_final:
    a, b, c = s
    add_parabola_to_plot(a, b, c+ 2/3, x_parabola_values, 'Parabola')
